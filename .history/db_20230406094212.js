...
export async function changeDataLogin() {
  ...
  export async function changeDataUs() {
    try {
        const database = client.db('User');
        const collection = database.collection('userInf');
        let query = {};
        // Lấy kết quả của hàm getData khi khởi động server
        resultUs = await getData(query);
        lengthUs = resultUs.length;
        console.log(length)
      // Sử dụng phương thức watch để theo dõi thay đổi trong bộ sưu tập
        const changeStream = collection.watch();
        while (true) {
            ...
            console.log('Thay đổi:', next);
            //update result
            // Cập nhật kết quả của hàm getData khi có thay đổi xảy ra
            resultUs = await getData(query);
            lengthUs = resultUs.length;
...
export {result,length,resultUs,lengthUs};
