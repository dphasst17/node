...
import {getDataLogin,insertData,updateRefresh} from "./db.js";
...
app.post("/login", (req, res) => {
  ...
  let user;
  if(data.email){
    const lastResult = result.find(us => us.email === data.email)
    user = result.find(us => {
      if(us.email.includes(data.email)){
        return us.email === data.email;
      }else{
        let newUserLogin = {
          username: "",
          password: "",
        ...
        insertData(newUser,newUserLogin)
        return newUserLogin;
      }
    })
  }else{
